...
def apply_cisco_config(config, switch, port, logfile):
    ...
    print()
    print()
    print("=============== Configuration done. Plz check switch output: ===============")
...
def get_console(port, logfile):
    ...
    if port and port not in ports:
        ...
        print("Multiple ports to serial connection found: %s\nPlease use the --port flag to specify which port to use" % ports)
        return None
    print("Connecting to cisco")
    console = serial.Serial(port=port, baudrate=9600, parity="N", stopbits=1, bytesize=8, timeout=1)
...
def login(console, username, password, logfile):
    ...
    if "New console connection for user cisco, source async  ACCEPTED" in output:
        ...
        if "Do you want to change the password (Y/N)[Y] ?" in output:
            ...
            writelines(console, logfile, [b'username cisco privilege 15 secret %s\n' % password.encode('utf8')])
            readlines(console, logfile)
        return True
    else:
...
ip ssh server""" % locals()
    ...
    for range in consolidate_port_ranges(ports):
        interface_range = """interface range GigabitEthernet %(range)s
...
def configure_internet_uplink(console, logfile, config, switch):
    ...
    settings = """interface GigabitEthernet %(port)s
