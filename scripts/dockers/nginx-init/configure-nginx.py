...
def adjust_nginx_config(upgrade, system_config={}):
    ...
    j.system.fs.writeFile(
        config % ("sites-enabled", "nginx.conf"), template.render(**system_config)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--upgrade",
        help="apply the upgrade nginx config",
        default=False,
        action="store_true",
    )
    ...
    j.system.fs.copyDirTree(
        "/opt/code/github/openvcloud_installer/scripts/install/web-files",
        "/opt/cfg/upgrade/",
    )
